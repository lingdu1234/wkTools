...
{
    let s = sys_dict_data::Entity::find().filter(sys_dict_data::Column::DictType.eq(req.dict_type));
...
/// add 添加
pub async fn add<C>(db: &C, add_req: AddReq) -> Result<String>
where
    C: TransactionTrait + ConnectionTrait,
{
    //  检查字典类型是否存在
    ...
    let _ = sys_dict_data::Entity::insert(user).exec(&txn).await?;
    txn.commit().await?;
...
pub async fn edit(db: &DatabaseConnection, req: EditReq) -> Result<String> {
    ...
    let s_s = sys_dict_data::Entity::find_by_id(uid.clone()).one(db).await?;
...
pub async fn get_by_type(db: &DatabaseConnection, dict_type: &str) -> Result<Vec<sys_dict_data::Model>> {
    let mut s = sys_dict_data::Entity::find();

    s = s.filter(sys_dict_data::Column::DictType.eq(dict_type));
...
pub async fn get_all(db: &DatabaseConnection) -> Result<Vec<sys_dict_data::Model>> {
    let s = sys_dict_data::Entity::find()
