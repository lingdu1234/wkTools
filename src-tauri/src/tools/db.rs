...
use crate::database::entities::{sys_dict_data, sys_dict_type};
...
pub async fn db_conn() {
    ...
    let db = match fs::File::open(db_path) {
    ...
    match DB.set(db) {
        Ok(()) => tracing::info!("database set OK"),
        Err(_) => tracing::info!("database set ERROR"),
    };
}
