        ...
        <div>
                            ...
                            <tr class="pure-table-odd">
                                ...
                                <td>
                                    <el-button size="small" type="info" style="width: 60px"
                                        ...
                                        @click="exportSampleCountData('A')">格式A
                                    </el-button>
                                    <el-button size="small" type="info" style="width: 60px"
                                        :disabled="exportOutData.SampleCountData_B.length === 0"
                                        @click="exportSampleCountData('B')">格式B
                                    </el-button>
                                </td>
                            </tr>
                            <!--           无效磁片统计-->
            ...
            <el-table v-loading="loading" border :data="dataList">
                ...
                <el-table-column label="医院名称" align="center" prop="hospital_name" :show-overflow-tooltip="true"
                    width="180" />
                <el-table-column label="条码" align="center" prop="sample_code" :show-overflow-tooltip="true"
                    width="150" />
        ...
        <el-form label-width="80px">
            ...
            <el-form-item label="统计选项:" v-if="CiPianOptions.isShow">
                <el-radio-group v-model="CiPianOptions.optionSelected">
                    <el-radio v-for="option in CiPianOptions.options" :key="option.label" :label="option.label">{{
                            option.tagName
                    }}
                    </el-radio>
                </el-radio-group>
            ...
            <el-form-item label="统计规则:" v-if="CiPianOptions.isShow">
                <!--          使用 ！== 注意类型-->
                <el-input :disabled="CiPianOptions.optionSelected !== '2'" v-model="CiPianOptions.optionValue"
                    placeholder=""></el-input>
            </el-form-item>
            <!--        本底统计选项-->
            <el-form-item label="统计选项:" v-if="BendiOptions.isShow">
                <el-radio-group v-model="BendiOptions.optionSelected">
                    <el-radio v-for="option in BendiOptions.options" :key="option.label" :label="option.label">{{
                            option.tagName
                    }}
                    </el-radio>
                </el-radio-group>
            ...
            <el-form-item label="统计规则:" v-if="BendiOptions.isShow">
                <!--          使用 ！== 注意类型-->
                <el-input :disabled="BendiOptions.optionSelected !== '2'" v-model="BendiOptions.optionValue"
                    placeholder=""></el-input>
            </el-form-item>
...
async function getList() {
    ...
    if (
        ...
        proxy.$modal.msgWarning('结束时间不能小于开始时间');
        loading.value = false;
        return;
    }
...
async function getInvalidSample() {
    if (queryParams.value.hospital_id == undefined) {
        proxy.$modal.msgWarning('请先选择医院后再操作');
        return;
    }
    ...
    const [sample_res, msg] = await invoke("get_sample_list", { ...qp })
    if (msg !== null) {
        ElMessage.error(msg)
        loading.value = false;
        return
    ...
    const [sample_r_res, msg] = await invoke("get_sample_result_list", { ...qp })
    if (msg !== null) {
        ElMessage.error(msg)
        loading.value = false;
        return
...
async function getBenDiRange() {
    ...
    let tQueryParams = { ...queryParams.value };
    tQueryParams.by_hospital = tongJiQueryParams.value.byHospital;
    tQueryParams.by_instrument = tongJiQueryParams.value.byInstrument;
    tQueryParams.by_test_group = tongJiQueryParams.value.byTestGroup;
    tQueryParams.by_regent_lot = tongJiQueryParams.value.byRegentLot;
    tQueryParams.time_option = tongJiQueryParams.value.byMonth ? 'M' : undefined;
    tQueryParams.by_test_name = tongJiQueryParams.value.byTestName;
    tQueryParams.options = BendiOptions.value.optionValue;
...
async function getAllSampleData() {
    ...
    const [{ list, test_names },msg] = await invoke("get_all_result",{req:queryParams.value});
